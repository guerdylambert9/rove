...
**Version:** 0.1 · **Companion to:** REQUIREMENTS.md
...
The current state: you have a working **front-end scaffold** — six screens, a booking flow, static data, no backend, no auth, no payments. That's a real asset. It means the product's shape is settled and you can now build the machinery underneath it.
...
```
Phase 0  Legal + insurance validation      (parallel, non-code, GATES launch)
   │
Phase 1  Backend foundations + auth        ← build this first in code
   │
...
Phase 4  Coverage verification + e-signed agreement   ← the trust core
   │
Phase 5  Identity verification / screening
   │
...
### Phase 1 — Backend foundations + auth *(build first in code)*
...
1. **Weeks 1–3:** Phase 0 in motion (broker + attorney) and Phase 1 backend/auth.
2. **Weeks 4–7:** Phases 2–3 — persisted bookings and real payments.
...
*Update this roadmap as phases complete. If Phase 0 comes back with a "no" on the insurance model, stop and re-plan before writing more code — that answer changes everything downstream.*
